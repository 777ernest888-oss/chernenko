...
// === ИНИЦИАЛИЗАЦИЯ ===
async function init() {  const loader = document.getElementById('loadingScreen');
  ...
  try {
    ...
    if (container) {
      container.innerHTML = '<div class="empty-state">⚠️ Ошибка загрузки данных. Проверьте консоль (F12).</div>';
    }
  } finally {
...
function applyBranding() {
  ...
  if (config.brand.welcomeTitle && titleEl) {
    titleEl.textContent = config.brand.welcomeTitle;
  }
  if (config.brand.welcomeSubtitle && subtitleEl) {
    subtitleEl.textContent = config.brand.welcomeSubtitle;
  }
}
...
function renderListings(data) {
  ...
  data.forEach((item, index) => {
    ...
    if (typeof item.price_from === 'number') {
      price = item.price_from < 1000
        ? item.price_from.toFixed(1)
        : (item.price_from / 1000000).toFixed(1);
    }
    ...
    const area = (typeof item.area_min === 'number' && typeof item.area_max === 'number')
      ? `${item.area_min}–${item.area_max} м²`
      : '';
...
function updateMapMarkers(items) {
  ...
  items.forEach(item => {
    ...
    let p = '?';
    if (typeof item.price_from === 'number') {
      p = item.price_from < 1000
        ? item.price_from.toFixed(1)
        : (item.price_from / 1000000).toFixed(1);
    }
   
    const popup = `<b>${escapeHtml(item.name)}</b><br>от ${p} млн ₽`;
    const m = L.marker([item.lat, item.lng]).addTo(map).bindPopup(popup);
    markers.push(m);
  });
...
function closeConsultModal() {
  ...
  document.getElementById('consultForm')?.reset();
    if (document.getElementById('detailsModal').classList.contains('hidden') &&
      document.getElementById('mapContainer').classList.contains('hidden')) {
    hideBack();
  }
}
